...
"Xhariep District Municipality"] },
  ...
  filter() {
    this.filterService.filter(
      ...
      console.log(data);
      this.tableData = data; // Assign the retrieved data to tableData
    });
  }
  ...
  applySecondFilter() {
    this.filter(); // Reuse the filter method
  }

  avg() {
    this.filter(); // Reuse the filter method for avg filtering
  }

  genderFilter() {
    this.filter(); // Reuse the filter method for gender filtering
  }
